"""Daily NSE F&O bhavcopy → option_chain + futures parquet ingestion.
...
Output trees:
    data/option_chain/<YYYY>/<MM>/<YYYY-MM-DD>.parquet  (CE/PE rows)
    data/futures/<YYYY>/<MM>/<YYYY-MM-DD>.parquet      (FUTSTK/FUTIDX rows)

Per specs/2026-04-29-expiry_pin_strike_reversal-plan.md Phase A2 + A3,
plus specs/2026-05-07-sub-project-9-brief-stock_futures_basis_convergence.md
data-engineering plan §11.
...
from tools.option_chain._nse_bhavcopy_client import (  # noqa: E402
    BhavcopyNotFound,
    download_bhavcopy,
    parse_bhavcopy,
)
...
_DEFAULT_FUTURES_OUT_ROOT = _REPO_ROOT / "data" / "futures"


def _parquet_path(out_root: Path, session_date: date) -> Path:
    """<out_root>/<YYYY>/<MM>/<YYYY-MM-DD>.parquet"""
    return (
        out_root
...
def _validate_futures(df: pd.DataFrame, session_date: date) -> None:
    """Sanity-check the futures slice. Tolerant — F&O bhavcopy holds ~500
    futures rows on a normal day; we only flag flat-zero days as failures.
    """
    if len(df) < 50:
        raise ValueError(
            f"too few futures contracts in bhavcopy for {session_date}: {len(df)} < 50"
        )
    if df["close"].isna().all():
        raise ValueError(f"all futures close prices null for {session_date}")
    bad_expiry = df[pd.to_datetime(df["expiry_date"]).dt.date < session_date]
    if len(bad_expiry) > 0:
        raise ValueError(
            f"{len(bad_expiry)} futures rows with expiry_date < session_date for {session_date}"
        )
...
def ingest_one_session(
    ...
    *,
    ...
    futures_out_root: Optional[Path] = _DEFAULT_FUTURES_OUT_ROOT,
) -> Path:
    """Download + parse + validate + write parquet for one session.

    Writes:
      <out_root>/<YYYY>/<MM>/<YYYY-MM-DD>.parquet           — option_chain
      <futures_out_root>/<YYYY>/<MM>/<YYYY-MM-DD>.parquet  — futures (FUTSTK/FUTIDX)

    Returns the option_chain parquet path. Raises BhavcopyNotFound for
    non-trading days and ValueError for malformed/incomplete data.

    Pass `futures_out_root=None` to skip futures writes (not used by the
    standard CLI — primarily for tests).
    ...
    futures_path = (
        _parquet_path(futures_out_root, session_date)
        if futures_out_root is not None else None
    )
    options_exists = out_path.exists()
    futures_exists = futures_path.exists() if futures_path is not None else True

    if skip_existing and options_exists and futures_exists:
        return out_path

    # If only one side is missing we still need to download once.
    raw = download_fn(session_date)
    result = parse_fn(raw, session_date)

    # Options (preserve original behaviour)
    if not (skip_existing and options_exists):
        df = result.rows
    ...
    # Futures (new)
    if futures_path is not None and not (skip_existing and futures_exists):
        fut_df = result.futures if result.futures is not None else pd.DataFrame()
        _validate_futures(fut_df, session_date)
...
def ingest_range(
    ...
    *,
    ...
    Returns a summary dict with counters covering both option_chain and
    futures writes. A session counts as "skipped_existing" only when BOTH
    parquets are already present (mirrors ingest_one_session semantics).
    ...
    while cur <= end:
        ...
        futures_path = (
            _parquet_path(futures_out_root, cur)
            if futures_out_root is not None else None
        )
        opt_done = skip_existing and out_path.exists()
        fut_done = (
            (futures_path is None)
            or (skip_existing and futures_path.exists())
        )
        if opt_done and fut_done:
        ...
        try:
            ingest_one_session(
                ...
                futures_out_root=futures_out_root,
            )
...
def main(argv: Optional[list] = None) -> int:
    ...
    parser.add_argument("--out-root", type=Path, default=_DEFAULT_OUT_ROOT,
                        help=f"option_chain parquet output root (default: {_DEFAULT_OUT_ROOT})")
    parser.add_argument("--futures-out-root", type=Path, default=_DEFAULT_FUTURES_OUT_ROOT,
                        help=f"futures parquet output root (default: {_DEFAULT_FUTURES_OUT_ROOT}). "
                             "Pass an empty string to disable futures writes.")
    parser.add_argument("--no-skip-existing", action="store_true",
                        help="re-ingest sessions whose parquet already exists")
    args = parser.parse_args(argv)

    skip_existing = not args.no_skip_existing
    futures_out_root = args.futures_out_root if str(args.futures_out_root) else None
