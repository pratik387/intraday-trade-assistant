# tools/sub9_research/phase2_nifty_next_50_sector_divergence_signature.py
#
# Phase 2 empirical signature for `nifty_next_50_sector_divergence_intraday_revert`.
...
#
# Hypothesis: Sector-divergence mean-revert was KILLed on top-100 (mega-caps
# already arbed by HFT / index funds / passive flows). The "becoming-large"
# cohort (rank 51-100) is less arbed than top-50 heavyweights, more
# institutional than small-cap. Same mechanism may have edge in this slice.
#
# This script measures the *signature* of stock-vs-sector intraday divergence
# mean-revert across NIFTY Next 50 names. It is a SIGNATURE MEASUREMENT script
...
#   - Universe: NIFTY Next 50 (rank 51-100 by market_cap_cr, mis_enabled=True), per
#     assets/nifty_next_50_universe.json. Each symbol carries a sector_id.
...
#
"""Phase 2 empirical signature — nifty_next_50_sector_divergence_intraday_revert."""
...
CONFIG: Dict[str, object] = {
    ...
    # Universe (large_cap only — NIFTY Next 50 by construction, rank 51-100)
    ...
    "out_csv":         _REPO_ROOT / "reports" / "sub9_sanity"
                                   / "_phase2_nifty_next_50_sector_divergence_signature.csv",
}
...
def load_universe(path: Path) -> Tuple[Dict[str, str], Dict[str, float]]:
    """Return (sym -> sector_id, sym -> market_cap_cr) for NIFTY Next 50 names.
...
def evaluate_day(
    ...
    # Restrict to NIFTY Next 50 universe AND eligible (per-date gate)
    day_df = day_df[
...
def run() -> int:
    print("=" * 80)
    print("Phase 2 empirical signature -- nifty_next_50_sector_divergence_intraday_revert")
    ...
    # 1. Load NIFTY Next 50 universe + sector map (drop _50 residual)
    _log("Loading NIFTY Next 50 universe ...")
    ...
    universe_set = set(sym_to_sector.keys())
    ...
    for (yy, mm) in months:
        ...
        # Restrict to NIFTY Next 50 (massive prune)
