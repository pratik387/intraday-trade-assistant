"""Phase 2 empirical signature: nifty_100_block_sell_afternoon_continuation_short.
...
Mechanism: NIFTY 100 large-cap MIS-eligible stocks with NSE block-deal SELL print of
...
Universe: NIFTY 100 from assets/nifty_100_universe.json filtered via
...
NIFTY100_PATH = _REPO_ROOT / "assets" / "nifty_100_universe.json"
SECTOR_MAP_PATH = _REPO_ROOT / "assets" / "stock_sector_map.json"
MONTHLY_DIR = _REPO_ROOT / "backtest-cache-download" / "monthly"
OUTPUT_CSV = _REPO_ROOT / "reports" / "sub9_sanity" / "_phase2_nifty_100_block_sell_afternoon_continuation_signature.csv"
...
def _load_nifty100_universe() -> Tuple[Set[str], Dict[str, str]]:
    """Returns (bare_symbols_set, sym_to_sector_id)."""
    with open(NIFTY100_PATH, encoding="utf-8") as f:
        d = json.load(f)
    items = d["universe"]
    bare: Set[str] = set()
...
def _load_block_signals(nifty100: Set[str]) -> pd.DataFrame:
    """Load block-deal events, filter to NSE NIFTY-100 prints with trade_value >= 25 cr.
...
def _load_5m_for_months(months: List[Tuple[int, int]], universe: Set[str]) -> pd.DataFrame:
    """Load monthly 5m feathers, restrict to NIFTY 100, normalize time/date."""
    frames = []
...
def main() -> int:
    print("Phase 2 empirical signature: nifty_100_block_sell_afternoon_continuation_short")
    ...
    nifty100, sector_map_pri = _load_nifty100_universe()
    sector_map_fb = _load_sector_map_fallback()
    print(f"  NIFTY 100 universe loaded: n={len(nifty100)}")
    ...
    block_df = _load_block_signals(nifty100)
    print(f"  NSE block prints (>= Rs {BLOCK_NOTIONAL_MIN_CR} cr) in NIFTY 100, full window: n={len(block_df)}")
    ...
    # 4) Bar data (load full window once -- NIFTY 100 only, fits in memory comfortably)
    months = _months_in_window(FULL_START, FULL_END)
    bars = _load_5m_for_months(months, nifty100)
    print(f"  5m bar rows (NIFTY 100, full window): {len(bars):,}")
    if bars.empty:
    ...
    # 8) Baseline records: NIFTY 100 (sym, d) with NO block print of any side >=25cr
    ...
    # Pre-filter bar_anchor to NIFTY 100 (already restricted but defensive)
    ba_iter = bar_anchor[bar_anchor["sym"].isin(nifty100)]
    n_total = len(ba_iter)
