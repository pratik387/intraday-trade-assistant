# tools/sub9_research/phase2_nifty_next_50_gap_up_low_volume_fade_signature.py
#
# Phase 2 empirical signature for `nifty_next_50_gap_up_low_volume_followthrough_fade_short`.
...
# and the original NIFTY 100 script:
#   tools/sub9_research/phase2_nifty_100_gap_up_low_volume_fade_signature.py
#
# Hypothesis: Gap-up low-vol fade was KILLed on top-100 (mega-caps arbed by
# HFT / pre-market flow). The "becoming-large" cohort (rank 51-100) is less
# arbed than top-50 heavyweights, more institutional than mid-caps. Same
# mechanism may have edge in this slice.
#
# This is a SHORT-fade setup on NIFTY Next 50 (rank 51-100 by market cap)
# large-cap stocks that gap UP >= +0.8% at 09:15 open AND have a first-hour
# (09:15-10:15) cumulative volume LESS THAN 0.7x the same-stock prior-5-day
# same-window mean.
...
#   5. NIFTY Next 50 list source-of-truth: assets/nifty_next_50_universe.json
#      (built from nse_all.json: market_cap_cr > 0 AND mis_enabled, rank 51-100).
...
"""Phase 2 empirical signature - nifty_next_50_gap_up_low_volume_followthrough_fade_short."""
...
CONFIG: Dict[str, object] = {
    ...
    "universe_json":  _REPO_ROOT / "assets" / "nifty_next_50_universe.json",
    ...
    "out_csv":        _REPO_ROOT / "reports" / "sub9_sanity" / "_phase2_nifty_next_50_gap_up_low_volume_fade_signature.csv",
}
...
def _load_universe(path: Path) -> List[str]:
    """Load NIFTY Next 50 universe from assets/nifty_next_50_universe.json.
...
def run() -> int:
    print("=" * 80)
    print("Phase 2 empirical signature -- nifty_next_50_gap_up_low_volume_followthrough_fade_short")
    print(f"Window: {CONFIG['window_start']} -> {CONFIG['window_end']}")
    print(f"Universe: NIFTY Next 50 (rank 51-100, large_cap, MIS=True) from {CONFIG['universe_json'].name}")
    ...
    # 1. Load NIFTY Next 50 universe list
    universe_syms = _load_universe(CONFIG["universe_json"])
    universe_set = set(universe_syms)
    _log(f"NIFTY Next 50 universe loaded: n={len(universe_syms)}")
    ...
    # Restrict to NIFTY Next 50 for speed
    daily_df = daily_df[daily_df["symbol"].isin(universe_set)]
    _log(f"  daily rows (NIFTY Next 50): {len(daily_df):,}  symbols: {daily_df['symbol'].nunique():,}")
    ...
    #    - filter to NIFTY Next 50 universe
    ...
    for (yy, mm) in months:
        ...
        if df.empty:
            _log(f"  {yy:04d}-{mm:02d}: 0 NIFTY-Next-50 rows")
            continue
