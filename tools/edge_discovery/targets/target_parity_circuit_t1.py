"""Parity gate Target: circuit_t1_fade_short reproduction on Holdout window.

T14 (2026-05-15) RESOLUTION: PARITY SKIPPED for this setup.
  - No sub8 holdout parquet exists for circuit_t1_fade_short (setup was added
    post-sub8). Script will raise FileNotFoundError on `reports/sub8_oos_holdout*`.
...
This script is kept for future use: if a circuit_t1 baseline parquet is
generated (e.g., by re-running the live config end-to-end on Holdout) and
`_live_status` is populated, the parity gate will activate automatically.

The live setup's Holdout statistics would be captured in
config/configuration.json (setups.circuit_t1_fade_short._live_status). This script
loads reports/sub8_oos_holdout_clean/circuit_t1_fade_short.parquet and compares.
...
def run_parity_circuit_t1() -> dict:
    ...
    if not pq_path.exists():
        pq_path = _REPO / "reports" / "sub9_iv_rank_holdout" / "circuit_t1_fade_short.parquet"
    if not pq_path.exists():
        raise FileNotFoundError(f"circuit_t1_fade_short parquet not found (checked sub8_oos_holdout*, sub9_iv_rank_holdout)")
