...
class EMA5AlertPullbackStructure(BaseStructure):
    ...
    def _build_plan(self, ctx: MarketContext, side: str, event: Optional[StructureEvent] = None) -> Optional[TradePlan]:
        ...
        try:
            ...
            logger.warning(
                f"[{ctx.symbol}] ema5_alert_pullback plan rejected: {e.reason} {e.details}"
            )
            return None
        ...
        return TradePlan(
            ...
            # 5-EMA pullback targets follow the alert-candle structure (T1 =
            # alert candle extreme, T2 = trailing-stop-managed). Preserve.
