"""Panic Crash Revert LONG detector — sub-project #9 (T5 deep down-spike bounce).

Thesis: An intraday crash of >=7% over 15 minutes (trailing-3-bar 5m return
<= -7%) on a volume burst, in a thin small/micro/unknown-cap with no
institutional liquidity to absorb it, is forced/panic selling that over-shoots
fair value into a vacuum. The over-shoot snaps back into the close. The DEPTH
matters: a shallow -4% dip continues (confirmed inverse); only the deep
capitulation reverts. Same illiquid over-extension -> reversion family as the
LIVE gap_fade_short, opposite tail.

Cell-locked (pure EOD-hold, no T1/T2 — validated research signature):
  cap_segment in {small_cap, micro_cap, unknown}
  x trailing-3-bar return <= -7%
  x current-bar volume >= 2x causal trailing-mean bar volume (strictly-prior
    bars that day; >= min_bars_before_signal prior bars — NO look-ahead, Lesson #25)
  x 09:45-14:00 IST
  x per-day turnover floor >= Rs 1.5cr
  x next-bar-open entry (Mode B), EOD square-off, MIS long.

Brief: specs/2026-06-03-brief-panic_crash_revert_long.md

DO NOT enable in live (`enabled: true`) until the OCI-pipeline coverage gate
(does it fire on 2023-24 cap=unknown?) + circuit-aware validation + paper A/B
pass — see brief Section 12.
"""
...
class PanicCrashRevertLongStructure(BaseStructure):
    """LONG entry on a deep intraday crash (<=-7% / 15min) + volume burst."""

    def __init__(self, config: Dict[str, Any]):
        super().__init__(config)
        self.structure_type = "panic_crash_revert_long"
        ...
        self.crash_r3_max = float(config["crash_r3_pct_max"])
        ...
        # qualifying deep spike (mirrors gap_fade_short).
    ...
    def detect(self, context: MarketContext) -> StructureAnalysis:
        ...
        # Cap segment gate (illiquid cohort).
        ...
        # Need the signal bar plus >= min_bars_before_signal STRICTLY-PRIOR
        # bars that day for the causal volume baseline (>= min+1 total bars,
        # and >= 4 so the trailing-3-bar return exists).
        ...
        # --- Trigger 1: trailing-3-bar (15-min) crash ---
        r3 = (cur_close / close_3ago) - 1.0
        if r3 > self.crash_r3_max:
            return _empty(f"r3={r3*100:.2f}% > crash_r3_pct_max={self.crash_r3_max*100:.2f}%")
        ...
        # Confidence proxy: deeper crash -> higher confidence. crash floor maps
        # to 0.0; -2x the floor maps to 1.0. Clamp to [0, 1].
        depth = (r3 - self.crash_r3_max) / max(abs(self.crash_r3_max), 1e-9)
        ...
        # Latch in detect() — runs in MainDetector worker process and survives
        # across bars within that worker (see gap_fade_short rationale).
    ...
    def plan_long_strategy(
        ...
        """Generate a LONG TradePlan (Mode B = next-bar-open entry).
        ...
        Exit structure (validated research = PURE EOD-HOLD, no T1/T2):
          ...
          - hard_sl = WIDE catastrophe stop (entry * (1 - catastrophe_stop_pct))
            — a blowup guard to cap MIS tail risk, NOT a strategy exit. It is
            wide enough to almost never trigger in the validated distribution.
          - targets: single far/EOD target (priced at the catastrophe-stop
            distance ABOVE entry) so the plan is well-formed; the real exit is
        ...
        # Single far/EOD target at +catastrophe distance above entry. EOD-hold
        # means the time_stop normally squares off before this is touched; it
        # exists only so the plan is geometrically well-formed (rr defined).
        ...
        try:
            ...
            assert_sl_outside_entry_zone(zone, hard_sl, "long")
            enforce_min_stop_distance(
                entry, hard_sl, self.config.get("min_stop_distance_pct"),
            )
        except PlanRejected as e:
            logger.warning(
                f"[{context.symbol}] panic_crash_revert_long plan rejected: "
    ...
    def calculate_risk_params(
        ...
        """WIDE catastrophe stop BELOW entry (long). Blowup guard, not a
        strategy exit — see plan_long_strategy docstring."""
        hard_sl = entry_price * (1.0 - self.catastrophe_stop_pct / 100.0)
        risk_per_share = max(entry_price - hard_sl, entry_price * 1e-4)
