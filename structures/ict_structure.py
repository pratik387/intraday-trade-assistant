...
class ICTStructure(BaseStructure):
    ...
    # ICTStructure does NOT participate in the StructureManager plan-generation
    # flow (production uses main_detector.MainStructureDetector instead). These
    # stubs exist solely to satisfy BaseStructure's @abstractmethod contract so
    # the class remains instantiable. The previous implementations referenced
    # StructureEvent.direction / .trade_plan attributes that do not exist on
    # the dataclass and would AttributeError if ever invoked.

    def plan_long_strategy(self, context: MarketContext) -> Optional[TradePlan]:
        """Not used for ICT — planning is handled by main_detector."""
        return None
