...
## Disabling a setup must never gate the legs that wind DOWN its positions (2026-08-13)
**Pattern:** I disabled `crash2d_revert_long` (enabled/paper_enabled = false) while it held 10 filled positions and 3 pending AMO entries. That flag fed `_eligible_multiday_setups`, and that one list drove THREE legs — new entries, entry-fill verification, AND exits. Result: the 10 filled positions lost their exit path entirely (NSE:CLSEL was due to square off that day and simply would not have been), and the 3 pending entries could neither fill nor be cleaned up, so they sat in the book showing "pending entry" forever. The user spotted it from the UI — "I see crash2d setups pending for entry on Tuesday" — not from any alert.
**Why:** Positions outlive the flag that created them. `_eligible_multiday_setups` already documented this exact hazard for `cb_state` ("a paused setup's EXISTING positions must still run their exits") but the enabled/paper_enabled flags bypassed that reasoning. The same defect was latent in the overnight book, where it is worse: the exit legs read the shared slot-pool path from `setups[0]`, so disabling the only overnight setup returns `[]` and the leg logs "no overnight setups active" and returns — stranding a REAL-MONEY position. Pausing close_dn via that flag was on the table for ~Sep-2026.
**Rule:**
1. **An enable/disable flag gates NEW exposure only.** Never let it gate exits, fill verification, reconciliation, or any wind-down path. Model the two sets separately: `_managed_*` (everything that could be holding something, flags ignored) vs `_eligible_*` (allowed to open new).
2. **Before flipping any setup off, enumerate what it is currently holding** — filled positions AND pending/unfilled orders — and confirm each has a path to resolution under the new flag state. "Disabled" is not a terminal state while inventory exists.
3. **Pending orders of a disabled setup are DROPPED, not filled.** Filling them opens new exposure in a setup you just switched off.
4. **Scope the "managed" predicate to execution-wired setups.** My first fix used `horizon == "multi_day"` alone, which swept in research-stage setups with no `capital_allocation` block; `_position_state_dir` would have raised KeyError while building the persistences map and aborted EVERY exit — strictly worse than the original bug. Filter on the config the wind-down path actually reads.
5. **When a bug is found in one book, grep the sibling books for the same shape before closing it.** The overnight copy of this defect was latent, real-money, and one config edit away from firing.
6. **Tests that isolate setups by patching a selector are pinned to that seam.** Splitting one selector into two means every such test must inject through both, or it silently stops covering the new path.

## Any "current book" number must be filtered to CURRENTLY-ACTIVE setups (2026-08-13)
**Pattern:** Twice in one session I reported a live-book figure that silently pooled retired setups. (1) Intraday P&L "−₹48,612" — but −₹47k of it was `gap_fade_short` + `below_vwap_volume_revert_long`, both already disabled; the live book was −₹1,643. (2) Capital utilisation "median peak 7.3% / mean 9.2% of ₹5L" — taken from `logs/*/capital_report.json`, which is a per-session AGGREGATE over whatever ran that day; the active book is 2.53% / 4.39%. `below_vwap` alone was ~65% of capital usage at ~₹100k notional × 110 trades. The user caught both.
**Why:** Per-session aggregate artefacts (`capital_report.json`, session summaries, pooled ledgers) have no setup dimension, so they answer "what did the book do then", never "what does the book do now". Reaching for them is convenient precisely when the question is about the current system — and retired setups are usually the ones with the extreme sizes and losses, so the contamination is large and always in the flattering-to-the-diagnosis direction.
**Rule:**
1. **Filter to `enabled or paper_enabled` FIRST, before computing any statistic** that will be described as the current book's. Print the active-setup list next to the number so the scope is visible and checkable.
2. **Do not use per-session aggregate reports for current-book questions.** Rebuild from per-trade rows (`analytics.jsonl`), which carry `setup_type`. If an aggregate is the only source, say so and label the number as historical.
3. **Validate the reconstruction against the aggregate** before trusting it (peak-concurrency rebuild gave ₹36,594 vs the report's ₹40,012 — close enough to trust the method, which is what licensed the active-only figure).
4. This is the same defect class as [pooling across a regime boundary](multiday archive routing). A statistic is only meaningful over ONE population — one regime, one setup mix. Name the population before computing.

## Aggregate P&L over ALL exit legs, never just the final one (2026-08-13)
**Pattern:** I reported the intraday book as flat-to-negative (net −₹1,643, PF 0.97) and built a whole argument on it — "scaling multiplies a zero", "size is anti-correlated with returns", "don't raise risk". The user said the ledger showed ≈+9.5k. They were right: I filtered `analytics.jsonl` to `is_final_exit` rows and summed `net_pnl`, which counts ONLY the last leg of a multi-exit trade. 15 of 120 trades took a partial T1 exit, and those partials are disproportionately WINNERS (T1 is the profit target). Correct net was **+₹10,182, PF 1.18**.
**Why:** `is_final_exit` is the right filter for counting TRADES (one row per position) but the wrong filter for summing MONEY. The bug is silent and one-directional: it always discards realised profit-taking, so a profitable book reads as flat and a flat one reads as losing. Worse, it corrupted derived claims invisibly — the "% return positive but rupees negative" divergence I treated as a real finding was purely this artifact, and I wrote it into a module docstring and commit message as justification for a design.
**Rule:**
1. **Sum P&L over every leg, grouped by `lifecycle_id`.** Use `is_final_exit` only to COUNT trades or to read terminal state (exit reason, exit price), never as a money filter.
2. **Sanity-check any computed P&L against the ledger/dashboard the user sees** before quoting it or building an argument on it. A mismatch is the cheapest possible bug detector and I had one available the whole time.
3. **A sign flip is a red flag, not a finding.** When % returns say positive and rupees say negative on the same trades, suspect the aggregation before inventing an economic story (anti-correlated sizing) to explain it.
4. **Do not write empirical numbers into docstrings/commits as design justification until they are reconciled against an independent source.** Structural justifications (undeclared sizing, two code paths, uncapped tail) survive a data bug; empirical ones do not.

## REPEAT of the final-leg bug, now in position SIZE too — and measure what the user actually watches (2026-09-15)
**Pattern:** The user said the intraday book "goes past ₹5k every day at some point before giving it back" and that my daily-target check "is broken somehow". It was. The curve reconstruction (`intraday_daily_target_study.py`, `intraday_daily_target_backtest.py`, and the OCI extract feeding it) filtered `analytics.jsonl` to `is_final_exit` rows — the exact bug recorded on 2026-08-13 above, one month earlier. This time it did more than drop T1 profit: the final row's `qty` is the LEFTOVER after the partial, so every T1-partial trade (19% of live trades, 10% of backtest trades) was carried at half size for its whole life and its pre-partial peak was halved. On top of that the curve was 5m-close and net-of-fees while the dashboard the user watches is 1m/tick and GROSS. Corrected live post-resize (15 sessions, 1m gross): 10 of 15 days crossed +₹5k; a ₹5k stop-for-the-day would have been +₹49k better, ₹3k +₹120k better. The backtest rerun is in the same commit.
**Why:** (1) A lesson that is only in `lessons.md` and not in the loader is not a fix. I re-derived the same extraction from scratch (`extract_bt.py`, `extract2.py`) and typed `if not t.get("is_final_exit"): continue` again because it "counts trades". (2) I validated the method against its OWN arithmetic (day-end reconciles ₹0.00) rather than against the number the user sees on screen. Self-consistency is not correctness; the dashboard was an independent oracle available the whole time. (3) I defended the conclusion ("definitively rejected") from the backtest before checking whether the measurement matched the user's observation of the live book.
**Rule:**
1. **`is_final_exit` NEVER filters rows for size, money, or curves.** One row = one exit LEG. Group legs by (symbol, entry price, session) / `lifecycle_id`; size = sum of leg qty; P&L = sum of leg `pnl`; fees = sum of leg (`pnl - net_pnl`). `tools/intraday_mtm_peak_from_events.py::load_analytics` is the reference loader — reuse it, do not re-derive.
2. **When the user reports seeing X on the dashboard, first reproduce X with the dashboard's own definition** (Live tab = closed gross + booked partials + LTP-marked unrealised, no fees, per tick) before arguing about it. If my number disagrees with what they saw, my measurement is the suspect, not their eyes.
3. **Bar closes are a floor, not a measurement, for any "did it touch T" question.** Use 1m bars (the engine's own family; 5m_enriched is contaminated) and report the high/low ceiling alongside the close.
4. **A repeat of a logged lesson is a process failure, not a data bug.** Before writing any analytics.jsonl reader, grep `lessons.md` for the field names being filtered on.

## "The 1m family is clean" was asserted from one day and was wrong (2026-09-15)
**Pattern:** On 2026-09-08 I found RMDRIP contaminated in the 5m_enriched feathers, checked ONE engine position (2026-04-08, which happens to be a clean day in the middle of the bad window), saw its MFE agree with the 1m ticks, and wrote into memory that "the 1m family is clean; the engine never saw it". Today the concentration check showed panic_crash_revert_long's three largest backtest trades are RMDRIP 03-30 / 04-02 / 04-09, each +35% in one minute because the T1 partial filled on the phantom ~Rs40 print. The 1m feather has the same two merged price series. Those three trades are 52% of the setup's 857-session net; without them its 2026 return is -0.10% and 2025 was already -0.76%.
**Why:** I verified the claim at the wrong granularity: one position on one day, not the symbol-window the 5m scan had flagged. And I let a convenient result ("engine never saw it") close the question instead of running the same scan on the family the engine actually replays. A claim written into memory then propagated: today's 1m-based tools cite "1m family clean" in their docstrings.
**Rule:**
1. **A data-quality claim about a file family is verified by scanning that family**, with the same scanner, not by spot-checking one trade. If the scanner was not run on it, the memory note says "unscanned", never "clean".
2. **Before quoting any setup's backtest edge, list its top-5 trades by P&L and check each against raw bars for the day** (entry/exit inside the day range, day range < 40%, return plausible for the hold time). A one-minute +35% in a Rs20 stock is a data error until proven otherwise.
3. **Concentration is a required line in any setup summary:** net after removing the top 5% of trades, per year. long_panic_gap_down and or_window_failure_fade_short go negative on that line in 3 of 4 years; that belongs next to their PF, not in a footnote.
